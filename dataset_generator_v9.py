# v9: Added static ROI filtering defined as a polygon in the config.
# Only detections whose center falls inside the ROI polygon are saved.
# Set ROI_POINTS to an empty list to disable and process the full frame.
...
# Define ROI as a list of (x, y) points. Set to [] to disable.
ROI_POINTS = [
    (300, 200),
    (1200, 200),
    (1200, 900),
...
def in_roi(cx, cy):
    if roi_contour is None:
        return True
    return cv2.pointPolygonTest(roi_contour, (float(cx), float(cy)), False) >= 0
